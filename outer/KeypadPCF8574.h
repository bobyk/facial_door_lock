...
// Матриця 4x3 через розширювач I2C PCF8574T. Неблокуючий: scan() лише читає
// поточний стан і повертає символ при новому натисканні (програмний дебаунс
// за millis()).
class KeypadPCF8574 {
public:
...
private:
    ...
    // Визначено емпірично на реальному підключенні - ribbon-кабель цієї
    // клавіатури не йде в порядку "рядки 0-3, стовпці 4-6".
    static constexpr uint8_t ROW_BIT[ROWS] = {5, 0, 1, 3};
    static constexpr uint8_t COL_BIT[COLS] = {4, 6, 7};
